...
public:
	...
	struct zombie_bg_master_finalAI : public ScriptedAI
	{
		...
		void SummonedCreatureDespawn(Creature* summon)
		{
			...
			case NPC_START_NEXT_WAVE:
				if (zombieWavesCount == zombieWaves) {	/* spawned all waves */
					...
					if (!isGroupDead()) {
						...
						Summons.DespawnAll();
						resetZombieEvent();
						TC_LOG_INFO(LOG_FILTER_GENERAL, "Everyone is dead. Stopping....2");
					}
				}
				break;
			...
			case NPC_WAIT_FOR_END_ROUND: /* called when the all waves are spawned but zombies are still left */
				if (isGroupDead())	/* check if the zombies killed all the players */
				{
					...
					Summons.DespawnAll();
					resetZombieEvent();
					TC_LOG_INFO(LOG_FILTER_GENERAL, "Everyone is dead. Stopping....3");
				}
				else {
		...
		{
			TC_LOG_INFO(LOG_FILTER_GENERAL, "teleportPlrsOut");
		...
		void spawnZombies(Creature* creature, uint32 count)
		{
			...
			for (uint32 i = 1; i <= count; i++) {
				...
				case 6:
					...
					break;
				default: TC_LOG_INFO(LOG_FILTER_GENERAL, "spawn default"); break;
				}
			}
		}
...
public:
	...
	{
		TC_LOG_INFO(LOG_FILTER_GENERAL, "OnGossipHello");
